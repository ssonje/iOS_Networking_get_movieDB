...
class NetworkingRealTimeGetAPITests: XCTestCase {
    ...
    func testNetworkFetchTrendingMovies() {
        ...
        NetworkService.shared.get.trendingMovies(completion: { (result: Result<TrendingMovies, Error>) in
            switch result {
    ...
    func testNetworkFetchPopularMovies() {
        ...
        NetworkService.shared.get.popularMovies(completion: { (result: Result<TrendingMovies, Error>) in
            switch result {
            case .success(let movies):
                XCTAssertNotNil(movies)
                expectation.fulfill()
